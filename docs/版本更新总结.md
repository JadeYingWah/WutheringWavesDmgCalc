...
> 生成: 2026-05-07 | 更新: 2026-06-12 | 版本: v1.0
...
- **v0.15**：预设同步多源 fallback——raw.githubusercontent.com + raw.gitmirror.com 自动重试，国内无需 VPN
- **v0.15**：预设同步错误日志——下载失败详情写入 error_log.json，侧边栏「错误日志」可查看具体原因

- **v0.15**：新增 tools/upload_presets.py 批量上传工具——自动扫描 presets/official/ 并上传到 GitHub，含 manifest.json 自动更新

---

## v1.0 正式版（2026-06-12）

v0.15 经过充分测试和优化，正式发布 v1.0。主要里程碑：

| 模块 | 状态 |
|------|------|
| OCR 系统 | PP-OCRv5 server 识别 + mobile 检测，15,000+ 字库，GPU 加速 |
| 预设系统 | 官方预设 GitHub 同步（多源 fallback），用户预设编辑/保存/应用 |
| 打包分发 | PyInstaller 一键打包，WutheringWavesDmgCalc 主文件夹结构 |
| 错误处理 | 全链路错误日志 + ErrorViewer 独立报告程序 |
| 版本管理 | Git + GitHub，预设上传工具，Release 发布 |

### v1.0 相比 v0.14 的核心变化

**OCR 系统重写**：
- 模型从 PP-OCRv3 mobile → PP-OCRv5 server（识别 81MB）+ mobile（检测 4.6MB）
- 字库覆盖 6,623 → 15,000+，游戏生僻字（抃、儛）全识别
- 速度优化：mobile 检测替代 server 检测（10x 加速）、det_limit 优化、DirectML GPU
- 全屏截图智能裁剪：声骸右上 1/4 + 倍率左 1/3
- COST 检测重写：固定词条反推 + 同行数字优先 + 多候选评分
- 噪声保护：词条名不受噪声过滤影响
- 4 种 OCR 场景统一输出逐行解析

**预设系统完善**：
- GitHub 官方预设同步——raw.githubusercontent.com + raw.gitmirror.com 双源 fallback
- 同步进度窗口分阶段显示 + 实时计数
- 同步失败详情写入错误日志
- 中文文件名 URL 编码修复
- 预设命名和存档命名非法字符校验
...
**打包与分发**：
- 输出结构重组为 WutheringWavesDmgCalc 主文件夹
- 快捷方式 + presets/save/config 数据目录
- after_build.py 自动化后处理
- 路径系统重写（_APP_DIR / _DATA_DIR 分离）
- ErrorViewer 独立打包 + 窗口图标修复
- 使用手册路径修复（_DATA_DIR → _MEIPASS）

**代码质量**：
- 退出弹窗 X 按钮修复
- 文件名非法字符校验
- 错误日志路径修复（打包后正确指向主文件夹）
- 批量上传预设工具 tools/upload_presets.py
