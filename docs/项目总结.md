...
| GitHub 集成 | ✅ 已完成 | 官方预设同步（多源 fallback），批量上传工具，Release 发布 |
...
├── ocr_engine.py                         # ★ OCR 图文识别引擎（~1600行，从主编拆分）
├── preset_manager.py                     # ★ 预设管理器（list/load/validate/apply/save/sync）
├── preset_builder.py                     # ★ 预设构建器（角色/武器/声骸套装/角色增益编辑窗口）
├── preset_loader.py                      # ★ 使用预设窗口（来源选择→分类→多选→应用）
│
├── tools/                                # 🔧 辅助工具
│   └── upload_presets.py                 #     批量上传预设到 GitHub
│
├── tests/                                # 🧪 自动化测试
│   ├── __init__.py
...
│
├── ico/                                  # 🖼 应用图标
│   ├── assemble_ico.py                   #     SVG → 多尺寸 .ico 渲染 + 合并脚本
│   ├── icon.ico                          #     最终图标文件（7 尺寸合并，31KB）
│   └── Wico.svg                          #     SVG 源文件
│
├── models/                               # 🧠 OCR 模型（不入 git）
│   ├── ch_PP-OCRv5_mobile_det.onnx       #     检测模型（4.6 MB）
│   └── ch_PP-OCRv5_rec_server_infer.onnx #     识别模型（81 MB）
│
└── dist/                                 # 🚀 分发包（PyInstaller 输出）
    └── WutheringWavesDmgCalc/            #     主文件夹
        ├── WWDmgCalc/                    #         主程序
        ├── ErrorViewer/                  #         错误报告
        ├── presets/                      #         预设
        ├── save/                         #         存档
