...
> 生成: 2026-05-30 | 更新: 2026-06-10 | 版本: v0.9

---

## 一、项目概览

**WutheringWavesDmgCalc** 是《鸣潮》(Wuthering Waves) 的伤害计算器桌面应用。基于 PyQt6，主模块 + 7 个拆分模块（主编 [WWDmgCalc.py](../WWDmgCalc.py) ~10185 行），支持暗色/亮色双主题、OCR 识别、存档管理、基础数值手动覆盖、错误日志系统、数据流调试器、使用手册。项目使用 **Git** 做版本管理，可随时回退到历史状态。
...
│  [数据流]     │  → DataFlowViewerDialog（数据流调试器，彩色 ID 追踪）
│ ─────────── │  分割线
│  导航树      │  5 大分类
└─────────────┘
```
...
ResultPage（计算结果页）汇总所有乘区：
        ...
        │  │  期望伤害：9,012     增伤倍率：117.5%       │
        │  │  计算过程：[HTML 格式，各乘区数值可点击跳转]│
        │  └────────────────────────────────────────────┘
        │
        │  用户点击"计入结果"：
        ▼
...
| `DataFlowViewerDialog` | 4401 | 数据流调试器（5 列：项目/数值/分类/序列号/副名称，彩色 ID 描边） |
