...
├── preset_builder.py                     # ★ 预设构建器（角色/武器/声骸套装编辑窗口）
...
预设系统允许用户创建、加载和管理角色/武器/声骸套装的预设配置。预设以 JSON 格式存储，分为官方预设（从 GitHub 同步）和用户预设（本地创建）。角色预设窗口内置独立计算引擎，可直接在预设内完成伤害计算并生成结果卡片。
...
| `preset_builder.py` | PresetBuilderDialog：角色/武器/声骸套装编辑器 + 适配器类 |
